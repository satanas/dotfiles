#!/bin/sh

set -e

RED="\033[0;31m"
GREEN="\033[0;32m"
YELLOW="\033[1;33m"
CYAN="\033[1;36m"
BLUE="\033[1;34m"
BOLD="\033[1;1m"
NO_FORMAT="\033[0m" # No Color

# Parse command line arguments
RUN_LINKS=false
RUN_BREW=false
RUN_ALL=true

for arg in "$@"; do
    case $arg in
        --links)
            RUN_LINKS=true
            RUN_ALL=false
            ;;
        --brew)
            RUN_BREW=true
            RUN_ALL=false
            ;;
        *)
            echo "Unknown option: $arg"
            echo "Usage: $0 [--links] [--brew]"
            echo "  --links: Only link dotfiles"
            echo "  --brew:  Only run brew update and install apps from Brewfile"
            echo "  (no flags): Run complete installation"
            exit 1
            ;;
    esac
done

error() {
    echo "${RED}Error: ${NO_FORMAT}$1"
    exit 1 || return 1
}

title() {
    echo "${BLUE}==>${NO_FORMAT} ${BOLD}$1${NO_FORMAT}"
}

ok() {
    echo "${GREEN}$1${NO_FORMAT}"
}

SOURCE_DIR=$(pwd)
DOTFILES=~/projects/dotfiles
BREWFILE=${DOTFILES}/config/Brewfile

# Oh My Zsh setup (needed for dotfiles linking)
if [ "$RUN_LINKS" = true ] || [ "$RUN_ALL" = true ]; then
    title "Setting up Oh My Zsh"
    # Check for Oh My Zsh and install if we don't have it
    if [[ ! -d ~/.oh-my-zsh ]]; then
        echo "Installing Oh My Zsh"
        curl -fsSL https://raw.githubusercontent.com/ohmyzsh/ohmyzsh/master/tools/install.sh || { error "✗ Unable to install Oh My Zsh."; exit 1; }
        ok "✓ OyMyZsh installed."
    else
        ok "✓ OyMyZsh already installed."
    fi
fi

if [ "$RUN_ALL" = true ]; then
    title "Setting up Homebrew"
    if test ! $(which brew); then
        echo "Installing Homebrew..."
        curl -fsSL https://raw.githubusercontent.com/Homebrew/install/master/install.sh || { error "✗ Unable to install Homebrew."; exit 1; }
        ok "✓ Homebrew installed."
    else
        ok "✓ Homebrew already installed."
    fi

    title "Setting up dotfiles repo"
    if [ -d "${DOTFILES}" ]; then
      echo "Dotfiles repo already present. Fetching latest changes..."
      cd "${DOTFILES}" && git pull origin master > /dev/null
      ok "✓ Dotfiles updated."
    else
      echo "Cloning dotfiles repo..."
      git clone https://github.com/satanas/dotfiles.git ${DOTFILES}
      cd ${DOTFILES}
      ok "✓ Dotfiles cloned."
    fi
fi

# Install apps first, so we have stow available
if [ "$RUN_BREW" = true ] || [ "$RUN_ALL" = true ]; then
    cd ~
    brew update
    title "Installing applications from ${BREWFILE}"
    brew bundle --file=${BREWFILE}
fi

# Link dotfiles
if [ "$RUN_LINKS" = true ] || [ "$RUN_ALL" = true ]; then
    # Removes .zshrc from $HOME (if it exists)
    rm -rf ~/.zshrc

    cd ${DOTFILES}/config

    title "Linking dotfiles"
    for filename in $(ls .);
    do
      if [ -d "${filename}" ]; then
        stow -R ${filename} -t ~ #2> >(grep -v 'BUG in find_stowed_path? Absolute/relative mismatch' 1>&2)
        echo "✓ Linked ${filename}"
      fi
    done;
fi

echo "${CYAN}Configuration completed successfully!${NO_FORMAT}"

# Switch to zsh (only when linking dotfiles)
if [ "$RUN_LINKS" = true ] || [ "$RUN_ALL" = true ]; then
    title "Switching to zsh"
    cd $SOURCE_DIR
    exec zsh
    source ~/.zshrc
fi
